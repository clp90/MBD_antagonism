...
### 10 07 2017
### adding the option to supply 2 (or more) filenames that are both used for deduplication. This is supposed to be an alternative to merging and mapping straight at the FastQ stage

### 10 03 2018
### Changing the strand identity from a single + and - to OT, OB, CTOT and CTOB. This may help discriminating between rare cases when OT and CTOB alignments (which are technically both + alignments)
...
sub print_helpfile{
  ...
  print "For paired-end alignments the chromosome, strand alignments, and start as well as end coordinate of the entire fragment will be used for deduplication. ";
  print "This script expects the Bismark output to be in SAM format\n(Bismark v0.6.x or higher). To deduplicate the old custom Bismark output please specify '--vanilla' (might be removed in future versions).\n\n";
  ...
  print "This script was last modified on 10 March 2018\n\n";
}
...
VERSION
    ...
    return ($single,$paired,$global_single,$global_paired,$vanilla,$samtools_path,$bam,$rrbs,$multiple,$output_dir);
}
